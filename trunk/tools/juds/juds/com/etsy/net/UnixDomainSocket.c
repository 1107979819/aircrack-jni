...
JNIEXPORT jint JNICALL
Java_com_etsy_net_UnixDomainSocket_nativeOpen(JNIEnv * jEnv, jclass jClass, jstring jSocketFile, jint jSocketType)
{
    int s;            /* socket file handle */
    struct sockaddr_un sa;
    const char *socketFile =
    ...
    if (connect(s, (struct sockaddr *)&sa, salen) == -1)
    {
		...
		int cls = close(s);
		ASSERTNOERR(cls == -1, "nativeOpen: close connect error socket");
		return -1;
    }
